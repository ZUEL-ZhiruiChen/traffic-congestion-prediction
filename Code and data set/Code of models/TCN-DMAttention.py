...
df = pd.read_csv('/Users/zipging/Downloads/论文/论文2/Code and data set/Data set/xh_taxi_labelled.csv') ##读取之前处理好的数据
...
# 进行单向LSTM
#bigru_out = Bidirectional(GRU(gru_units, activation='relu', return_sequences=True))(x)
...
#test_loss = model.evaluate(test_x, test_y)  # 评估模型
    ...
    m = sklearn.metrics.classification_report(test_y.argmax(1), y_pred.argmax(1), digits=6, output_dict=True)
    m = pd.DataFrame(m)
...
##对比
matPred = np.array(matPred.values)
matTrue = np.array(matTrue.values)
compare = np.ones((matTrue.shape[0], matTrue.shape[1]))
for i in range(matTrue.shape[0]):
    for j in range(matTrue.shape[1]):
        if (matTrue[i, j] == matPred[i, j])&(matTrue[i, j]!=3):
            compare[i, j] = 1
            continue
        if matTrue[i, j] != matPred[i, j]:
            compare[i, j] = 0
            continue
        if (matTrue[i, j] == matPred[i, j])&(matTrue[i, j]==3):
            compare[i, j] = 2
...
cmap = clist=['#FFB90F','#1874CD','white']

plt.figure(figsize=(20, 10), dpi=300)
ticks = [0, 1]
sns.heatmap(compare[:190,30:], cmap=cmap,  cbar_kws={"ticks":[0, 1]}, xticklabels=False, yticklabels=False, square=True)
#plt.title('(b)Predicted value', fontsize=25, y=-0.06)
plt.savefig('预测对比', dpi=1000, bbox_inches='tight')
#plt.legend(['#8E8E38', '#1874CD'], ['#8E8E38', '#1874CD'] )
plt.show()