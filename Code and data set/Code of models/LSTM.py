...
# 进行单向LSTM
lstm_out = LSTM(64, activation='relu', return_sequences=False)(input)
output = Dense(output_dim, activation='softmax')(lstm_out)
model = Model(inputs=input, outputs=output)
# 模型编译
model.compile(loss='sparse_categorical_crossentropy', optimizer='sgd',metrics=['acc'])  # 编译模型
...
print(sklearn.metrics.classification_report(test_y,y_pred.argmax(1),digits=6))
sklearn.metrics.roc_auc_score(test_y,y_pred,average='weighted',multi_class='ovo')